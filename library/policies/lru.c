...
int cache_predicate_lru(const void * data, const void * search) {
    return strcmp(((cache_entry_t *) data)->ce_source, search) == 0;
}

unsigned long cache_transform_lru(unsigned long key) {
    return time(NULL);
}
...
void cache_init_lru(cache_t * cache) {
    ...
    init_heap(&cache->c_cache_entries, 4096, cache_transform_lru, cache_predicate_lru);
    ...
    if (result == 0) {
        ...
        for (int i = 0; i < 1024; i++) {
            ...
            if (cache->c_mapping[i] != NULL && cache->c_mapping[i]->cm_file == -1 && strcmp(entry->ce_source, cache->c_mapping[i]->cm_source) == 0) {
                ...
                strcpy(cache->c_mapping[i]->cm_cache, entry->ce_cache);
            }
            pthread_mutex_unlock(&cache->c_mutex_mapping[i]);
        }
