...
result_t * benchmark(const int runtime, const int files, const int window, const int iterations, const int blocksize, const int patterns, const int filesize, const int seed) {
  ...
  while (time(NULL) < start_time + runtime) {
    ...
    int file = open(path, O_DIRECT | O_RDONLY);
    if (file == -1) {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      ssize_t bytes_read = read(file, buffer, blocksize);
      if (bytes_read == -1) {
        perror("Error: could not read benchmark data.");
        exit(EXIT_FAILURE);
      }

      result->r_bytes += bytes_read;
      result->r_ops += 1;
...
int main() {
  ...
  result_t * result = benchmark(300, 32, 10, 1024, 65536, 1024, 67108864, 1234567890);
